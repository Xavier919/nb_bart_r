...
run_cross_validation <- function(data, formula, k = 5, random_effect = NULL, exposure = NULL) {
  set.seed(42)
  ...
  for (i in 1:k) {
    ...
    model <- SpatialMixedNegativeBinomial$new(
      ...
      exposure = NULL,  # Not using exposure
      spatial_vars = c('x', 'y')
    )
...
# Use the formula from feature selection
my_formula <- feature_results$formula
...
# Or create and fit a single model
model <- SpatialMixedNegativeBinomial$new(
  data = full_data,
  formula = my_formula,

...
run_combined_cross_validation <- function(data, response_var = "acc", k = 5, random_effect = "borough", max_features = 30) {
  ...
  # --- Preparations for SEM and INLA ---
  # Create spatial object for SEM
  sp_data <- data
  coordinates(sp_data) <- ~ x + y
  w_global_sem <- make_spatial_weights_for_sem(sp_data, k = 3)
  
  # Create sparse spatial weights matrix for INLA (using full data)
  coords_inla <- as.matrix(data[, c('x', 'y')])
  # Use the existing create_spatial_weights function, then make it sparse
  W_dense_inla <- create_spatial_weights(data.frame(x=coords_inla[,1], y=coords_inla[,2]), k_neighbors=3)
  W_sparse_inla <- Matrix(W_dense_inla, sparse = TRUE)
  
  # Define INLA priors and control settings once
  ...
  # Initial values for control.mode (Order: NB theta (log), BYM2 prec (log), BYM2 phi (logit), IID prec (log))
  ...
  for (i in 1:k) {
    ...
    # Exclude non-feature columns, including INLA's spatial index
    ...
    carbayes <- carbayes_model$new(
      data = train_data,
      formula = formula_str, # Use the base formula string
    ...
    if (inherits(carbayes_results, "try-error")) {
        ...
        carbayes_train_mae <- NA; carbayes_train_mse <- NA; carbayes_train_rmse <- NA
        ...
        carbayes_train_preds <- carbayes$predict(train_data) # Predict on train for train metrics
        carbayes_train_mae <- mean(abs(train_data[[response_var]] - carbayes_train_preds), na.rm = TRUE)
        carbayes_train_mse <- mean((train_data[[response_var]] - carbayes_train_preds)^2, na.rm = TRUE)
    ...
    # Create spatial data for SEM model for this fold
    train_sp <- sp_data[train_idx, ]
    test_sp <- sp_data[test_idx, ]
    w_train_sem <- make_spatial_weights_for_sem(train_sp, k = 3) # Fold-specific weights for SEM training
    
    sem_test_preds <- try(train_poisson_sem(
      formula = formula, # Use the formula object
      train_data = train_sp, 
      val_data = test_sp, 
      global_weights = w_global_sem, # Use global weights for prediction
      fold_weights = w_train_sem     # Use fold weights for training
    ), silent = TRUE)
    
    if (inherits(sem_test_preds, "try-error") || any(is.na(sem_test_preds))) {
        cat("ERROR or NA predictions in SEM model for fold", i, "\n")
        sem_test_mae <- NA; sem_test_mse <- NA; sem_test_rmse <- NA
        sem_test_preds <- rep(NA, nrow(test_data)) # Ensure it has the correct length for storage
    } else {
    ...
    # Prepare data for INLA: use full data, set test response to NA
    data_cv_inla <- data
    y_cv_inla <- data[[response_var]]
    y_cv_inla[test_idx] <- NA
    data_cv_inla$y_cv_inla <- y_cv_inla # Add the response vector with NAs
    
    # Construct INLA formula
    # Ensure random_effect column exists and is a factor (checked in load_data)
    inla_formula_str <- paste(
        "y_cv_inla ~", formula_str_base, # Use base formula (features or intercept)
        "+ f(idx_spatial, model = 'bym2', graph = W_sparse_inla, scale.model = TRUE, constr = TRUE, hyper = list(prec = pc_prec_unstructured, phi = pc_phi))",
    ...
    inla_model <- try(inla(inla_formula,
                           data = data_cv_inla, # Use the data with y_cv_inla
                           family = "nbinomial",
                           control.predictor = list(compute = TRUE, link = 1), # Compute predictions for NAs
    ...
    if (inherits(inla_model, "try-error")) {
        ...
        # Extract predictions for the test set indices
        predicted_mean_inla <- inla_model$summary.fitted.values$mean
        inla_test_preds <- predicted_mean_inla[test_idx]
    ...
    carbayes_fold_results[[i]] <- list(
      fold = i,
      train_mae = carbayes_train_mae, train_mse = carbayes_train_mse, train_rmse = carbayes_train_rmse,
  ...
  # Feature consistency analysis (based on CARBayes/LASSO selection)
...
comparison_df <- data.frame(
  ...
  Overall_MAE = c(cv_results$carbayes_results$overall_mae, cv_results$sem_results$overall_mae, cv_results$inla_results$overall_mae),
  Overall_MSE = c(cv_results$carbayes_results$overall_mse, cv_results$sem_results$overall_mse, cv_results$inla_results$overall_mse),
  Overall_RMSE = c(cv_results$carbayes_results$overall_rmse, cv_results$sem_results$overall_rmse, cv_results$inla_results$overall_rmse)
)
